...
namespace ArtistKit {
    ...
    public partial class UnitMaterialEditor : ShaderGUI {
                                ...
                                foreach ( var ig in includeGroups ) {
                                    ...
                                    if ( ed != null ) {
                                        if ( includeGroup != DefaultGroupName ) {
                                            ...
                                            for ( int g = 0; g < ed.tree.children.Count; ++g ) {
                                                ...
                                                if ( refEditors.editors != null && refEditors.editors.Count > 0 ) {
                                                    ...
                                                    if ( addTarget == null ) {
                                                        ...
                                                        Debug.Assert( addTarget == group );
                                                        group.editors.InsertRange( insertStartPos, refEditors.editors );
        ...
        protected MaterialProperty FindCachedProperty( String propertyName, MaterialProperty[] properties ) {
        ...
        protected MaterialProperty FindCachedProperty( String propertyName, MaterialProperty[] properties, bool propertyIsMandatory ) {
                        ...
                        for ( int i = 0; i < vals.Count; ++i ) {
                            ...
                            if ( val != null && val.IsObject ) {
                                ...
                                if ( s_ShaderPropGUIFactory.TryGetValue( editor, out f ) ) {
                                    ...
                                    if ( args != null && args.IsObject ) {
                                        args.GetField( out propName, Cfg.Key_Name, editor );
                                    }
        ...
        protected virtual void OnMaterialChanged( Material material, MaterialProperty[] props ) {
            for ( int i = 0; i < m_props.Count; ++i ) {
                m_props[ i ].OnMaterialChanged( material, props );
            }
        }
        ...
        void MaterialChanged( Material material, MaterialProperty[] props ) {
            OnMaterialChanged( material, props );
        }

        static bool Foldout( bool display, string title, out bool shift ) {
            shift = false;
        ...
        public override void OnGUI( MaterialEditor materialEditor, MaterialProperty[] props ) {
            ...
            if ( m_template == null && s_curMaterial != null && !IsEmptyShader( s_curMaterial.shader ) ) {
                if ( !s_templateCache.TryGetValue( s_curMaterial.shader, out m_template ) ) {
                    m_template = new Material( s_curMaterial.shader );
                    s_templateCache.Add( s_curMaterial.shader, m_template );
                }
            }
            ...
            if ( m_userDefaultEditor ) {
                ...
                if ( this.m_FirstTimeApply ) {
                    MaterialChanged( material, props );
        ...
        public T FindPropEditor<T>( String id = null ) where T : UnitMaterialEditor {
            ...
            if ( props != null ) {
                ...
                for ( int i = 0; i < props.Count; ++i ) {
                    if ( t.IsInstanceOfType( props[ i ] ) &&
                        ( id == null || id == props[ i ].m_id ) ) {
                        return props[ i ] as T;
                    }
                }
            }
        ...
        public virtual String GetReturnValue( MaterialProperty[] props ) {
            String returnValue;
            if ( GetLogicOpResult( out returnValue, props ) ) {
                return returnValue;
            }
            return String.Empty;
        }

        public bool? TryGetBoolTestResult( Material material, MaterialProperty[] props, String key = null ) {
            String _dummy;
            if ( material != null && m_args != null && m_args.GetField( out _dummy, key, String.Empty ) ) {
                if ( !String.IsNullOrEmpty( _dummy ) ) {
                    return GetBoolTestResult( material, props, key );
                }
            }
            return null;
        }

        public virtual bool GetBoolTestResult( Material material, MaterialProperty[] props, String key = null ) {
            if ( m_args != null ) {
                String _ref;
                    ...
                    if ( m.Success && m.Groups.Count > 2 ) {
                        ...
                        if ( !String.IsNullOrEmpty( id ) ) {
                            ...
                            if ( ui != null ) {
                                ...
                                } else if ( material != null && material.shader != null ) {
                                    var shader = material.shader;
