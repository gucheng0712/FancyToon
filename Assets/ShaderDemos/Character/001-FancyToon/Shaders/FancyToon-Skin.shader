	...
	SubShader
	{
		...
		Pass
		{
			...
			float4 frag(v2f f) : SV_Target
			{
				...
				float NdotV = dot(worldNormalDir, worldViewDir);
				float3 rim = _Shininess * pow(saturate(NdotV*NdotL), _Smoothness);

				// 使用邻域像素之间的近似导数值来对smoothstep实现抗锯齿的效果 
				float ramp = smoothstep(_ShadowOffset, _ShadowOffset+0.1, NdotL)*_LightColor0;

				float3 diffuse = rim * _RampColor.rgb +  albedo * (ramp + _ShadowIntensity);

				float specRange = (1-NdotV) * pow(NdotL, 0.1);
