Shader "Toon/Caustics Interactive Water" {
	Properties{
		...
		Subshader{
			...
			Pass {
				...
				fixed4 frag(v2f i) : SV_Target
				{
					...
					float2 uv = i.worldPos.xz - _RipplePosition.xz;
					uv = uv / (_OrthographicCamSize * 2);
					uv += 0.5;
					float ripples = tex2D(_GlobalEffectRT, uv).b;
					//float4 ripples = tex2D(_GlobalEffectRT, float2 (uv.x, uv.y));
					float4 efMask = tex2D(_Mask, uv);
					ripples = step(0.8, ripples);
					//ripples *= efMask.a;
					...
					float distort = triplanar(blendNormal, distortx + (ripples* 4) , distorty + (ripples * 4), distortz + (ripples * 4));
					// moving caustics
					...
					return saturate(causticsTex + _Color+ripples);
				}
				ENDCG
			}
		}
